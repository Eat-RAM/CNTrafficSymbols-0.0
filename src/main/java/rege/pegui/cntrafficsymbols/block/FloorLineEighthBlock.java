...
implements ManagedWaterloggable, StateOptimizable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        byte repl = 1;
        if (ctx.getStack().is(this.itm2)) {
            repl++;
        } else if (!ctx.getStack().is(this.itm1)) {
    ...
    public BlockState getStateForNeighborUpdate(
        ...
        return super.getStateForNeighborUpdate(
            state, world, tickView, pos, direction, neighborPos, neighborState,
            random
        );
    }
    ...
    public boolean canReplace(BlockState state, ItemPlacementContext ctx) {
        if (!(ctx.getStack().is(this.itm1) ||
              ctx.getStack().is(this.itm2))) {
            return false;
        }
        Direction d = ctx.getSide();
        BlockPos p = ctx.getBlockPos();
